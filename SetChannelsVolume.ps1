...
Add-Type -Language CsharpVersion3 -TypeDefinition @'
    ...
    public static class AudioManager
    {
        ...
        /// <param name="level">The new master volume level expressed as a normalized value between 0 and 100.</param>
		public static void SetChannelVolumeLevel(uint channel, float level)
		{
			IAudioEndpointVolume masterVol = null;
			
			if (channel >= GetChannelsCount())
			{
				return;
			}
            ...
            try
    		{
    			...
    			masterVol.SetChannelVolumeLevelScalar(channel, level/100, Guid.Empty);
    		}
    		finally
    		{
    			if (masterVol != null)
        ...
        public static float GetChannelVolumeLevel(uint channel)
        {
            IAudioEndpointVolume masterVol = null;
            ...
            try
    		{
    			...
    			float channelVolumeLevel;
    			masterVol.GetChannelVolumeLevelScalar(channel, out channelVolumeLevel);
    			return channelVolumeLevel*100;
    		}
    		finally
    		{
    			if (masterVol != null)
        ...
        /// </summary>
        /// <returns>-1 in case of an error, if successful the value will be the number of channels.</returns>
        public static uint GetChannelsCount()
        {
            IAudioEndpointVolume masterVol = null;
            try
    		{
    			...
    			uint channelCount;
    			masterVol.GetChannelCount(out channelCount);
    			return channelCount;
    		}
    		finally
    		{
...
$vProvided = $PSBoundParameters.ContainsKey('Volume')

if ($vProvided)
{
	if ($Volume -gt 100 -or $Volume -lt 0)
	{
		throw "Volume must be between 0 and 100!"
	}
}

$cCount = [AudioManager]::GetChannelsCount()
$mVolume = [Math]::Round([AudioManager]::GetMasterVolume())

echo "Channels count - $cCount"
echo "Current master volume - $mVolume%"
if ($vProvided) { echo "Set master volume to - $Volume%" }
echo ""

for ($cNum = 0; $cNum -lt $cCount; $cNum++)
{
	$cVolume = [Math]::Round([AudioManager]::GetChannelVolumeLevel($cNum))
	$sVolume = if ($vProvided) {$Volume} else {$mVolume}
	
	if ($cVolume -ne $sVolume)
	{
		echo "Channel $($cNum + 1) volume - $cVolume%, set it to $sVolume%"
		[AudioManager]::SetChannelVolumeLevel($cNum, $sVolume)
	}
	else
	{
		echo "Channel $($cNum + 1) volume - $cVolume%, skip"
	}
}